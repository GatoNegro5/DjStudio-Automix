...
class _LabWorkspaceState extends ConsumerState<LabWorkspace> {
  ...
  // ---------------------------------------------------------
  // 🛠️ MOTOR DE RESCATE ATÓMICO (100% DART - CROSS-PLATFORM)
  // ---------------------------------------------------------
  Future<void> _rescueLabTrackFromYoutube(
    ...
    final ValueNotifier<String> statusNotifier = ValueNotifier<String>(
      "Iniciando conexión Multiplataforma...",
    );
    ...
    try {
      ...
      // YouTube entrega la máxima calidad en formato WebM (Audio Opus). Es más ligero y de mejor calidad que el MP3.
      final tempAudio =
          '${tempDir.path}${Platform.pathSeparator}lab_rescue_temp.webm';

      if (File(tempAudio).existsSync()) {
        try {
          File(tempAudio).deleteSync();
        } catch (_) {}
      }

      statusNotifier.value =
          "1/6. Descargando stream crudo nativamente (Dart I/O)...";

      // 🛠️ BYPASS MULTIPLATAFORMA: Descarga de audio sin yt-dlp ni FFmpeg
      final manifestStream = await yt.videos.streamsClient.getManifest(videoId);
      final streamInfo = manifestStream.audioOnly.withHighestBitrate();
      final audioStream = yt.videos.streamsClient.get(streamInfo);

      final fileStream = File(tempAudio).openWrite();
      await audioStream.pipe(fileStream);
      await fileStream.flush();
      await fileStream.close();

      statusNotifier.value = "2/6. Extrayendo y purificando subtítulos VTT...";
      final ccManifest = await yt.videos.closedCaptions.getManifest(videoId);

      String? tempLrcPath;
      if (ccManifest.tracks.isNotEmpty) {
        ClosedCaptionTrackInfo? selectedTrack;
        for (var lang in ['es', 'en']) {
          try {
            selectedTrack = ccManifest.tracks.firstWhere(
              (t) =>
        ...
        selectedTrack ??= ccManifest.tracks.firstWhere(
          ...
          orElse: () => ccManifest.tracks.first,
        );
        ...
        if (lrcBuffer.isNotEmpty) {
          tempLrcPath = tempAudio.replaceAll('.webm', '.lrc');
      ...
      // Actualizamos la extensión del archivo destino si venía como .mp3 a .webm
      final newTargetFile = targetOriginalPath.replaceAll(
        RegExp(r'\.mp3$', caseSensitive: false),
        '.webm',
      );
      final targetOriginalFile = File(targetOriginalPath);
      final newFile = File(newTargetFile);
      final targetLrcFile = File(newTargetFile.replaceAll('.webm', '.lrc'));

      await File(tempAudio).copy(newFile.path);
      await File(tempAudio).delete();

      // Si el archivo original era .mp3, lo borramos ya que lo hemos reemplazado por la versión .webm
      if (targetOriginalPath.toLowerCase().endsWith('.mp3') &&
      ...
      statusNotifier.value = "5/6. Compensación del Vector de Tiempo (LRC)...";
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('✅ Rescate atómico nativo completado.'),
            backgroundColor: Color(0xFF39FF14),
          ),
        );
      }
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('🔴 Fallo de Rescate Nativo: $e'),
            backgroundColor: Colors.redAccent,
